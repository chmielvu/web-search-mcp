...
def _stage_outcome(
    ...
    *,
    ...
    updated = [
        candidate.model_copy(update={"cross_encoder_score": 0.9 - index / 1000} if cross else {})
    ...
    return RankedStageOutcome(
        ...
        provider=provider,
        model=model,
        stage_name="cross_encoder" if cross else "rankllm",
        input_count=len(candidates),
        output_count=len(candidates),
        duration_seconds=0.0,
        relevance_scores=[0.9 - index / 1000 for index in range(len(candidates))],
        max_score=0.9,
        attempted_passes=0 if cross else 1,
        valid_passes=0 if cross else 1,
    )
...
async def test_strict_boundaries_preserve_rankllm_window_and_every_tail(
    ...
    assert len(output.overflow_items) == candidate_count - 15
    assert (
        len(
            {candidate.link for candidate in output.results}
            | {item.result.link for item in output.overflow_items}
        )
        == candidate_count
    )
...
async def test_provider_consensus_bm25_fusion_keeps_all_score_identities() -> None:
    ...
    async def identity_rerank(_query, candidates, **_kwargs):
        return RerankOutput(
            results=[
                candidate.model_copy(update={"final_score": candidate.retrieval_rrf_score})
    ...
    assert all(result.final_score == result.retrieval_rrf_score for result in response.results)
