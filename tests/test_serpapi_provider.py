...
class TestSerpApiEngineOverride(unittest.TestCase):
    @patch(
        "kindly_web_search_mcp_server.search.serpapi._get_serpapi_api_key",
        return_value="key",
    )
    @patch(
        "kindly_web_search_mcp_server.search.serpapi._search_one_engine",
        new_callable=AsyncMock,
    )
    def test_engine_parameter_overrides_config(self, search_mock, *_):
        import asyncio

        from kindly_web_search_mcp_server.search.serpapi import search_serpapi
