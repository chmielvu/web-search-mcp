...
class TestBrightDataEngineUrls(unittest.TestCase):
    ...
    def test_yandex_url_includes_region_and_brd_json(self):
        from kindly_web_search_mcp_server.search.brightdata_common import build_yandex_url
...
class TestBrightDataAliasRouting(unittest.TestCase):
    ...
    def test_yandex_alias_calls_run_provider_with_name(self, *_):
        ...
        with patch("kindly_web_search_mcp_server.search.brightdata.run_provider", run_mock):
            results = _run_async(
                search_brightdata("test", num_results=5, provider_name="brightdata_yandex")
            )
            self.assertEqual(run_mock.call_args[0][0], "brightdata_yandex")
        self.assertEqual(len(results), 1)
