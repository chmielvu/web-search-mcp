...
def test_user_template_has_named_slots_and_no_specialized_block() -> None:
    for token in ("<FREE_QUERY_RULES>", "<SERP_QUERY_RULES>", "<TAVILY_QUERY_RULES>",
                  "<EXA_QUERY_RULES>", "{compared_entities}", "{time_sensitivity}",
                  "{should_decompose}", "{preserved_terms}"):
...
def test_named_model_parses_and_rejects_extras() -> None:
    payload = (
        '{"free":"a","serp1":"b","serp2":"c","semantic_tavily":"d","semantic_exa":"e"}'
    )
...
def test_free_fallback_prefers_segmented_variant() -> None:
    ...
    fb = _branch_fallback_queries(feats, terms=(), suggestions=(), research_goal="find one", current_year="2026")
    assert fb[0].startswith("top lawyers in new york")


def test_time_gated_year_suffix() -> None:
    base = _features("rust async runtime")
    cold = _branch_fallback_queries(base, terms=(), suggestions=(), research_goal="", current_year="2026")
    hot_feats = _features("rust async runtime", time_sensitivity="recent")
    hot = _branch_fallback_queries(hot_feats, terms=(), suggestions=(), research_goal="", current_year="2026")
    assert "2026" not in cold[0]
    assert "2026" in hot[0]


def test_serp2_uses_compared_facet_and_diverges_from_serp1() -> None:
    feats = _features("fastapi vs starlette performance", compared_entities=["FastAPI", "Starlette"])
    fb = _branch_fallback_queries(feats, terms=("kw1",), suggestions=(), research_goal="pick one", current_year="2026")
