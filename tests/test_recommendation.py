...
def test_known_url_summary_routes_to_content_get_with_ai_summary() -> None:
    result = build_command_recommendation("Summarize https://docs.example.com/guide")

    assert result.intent == "known_url_read"
    assert result.recommended_route.mcp_tool == "get_content"
    assert "content get" in result.recommended_command
...
def test_multiple_known_urls_routes_to_bounded_batch_content() -> None:
    result = build_command_recommendation(
        "Read https://example.com/a, https://example.com/b, and https://example.com/c"
    )

    assert result.intent == "multi_url_read"
    assert result.recommended_route.mcp_tool == "batch_get_content"
    assert result.recommended_route.arguments["--url"] == [
        "https://example.com/a",
        "https://example.com/b",
        "https://example.com/c",
    ]
