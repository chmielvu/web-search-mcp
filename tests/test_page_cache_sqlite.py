"""Tests for SQLite-backed page cache."""
...
class TestPageCacheSQLite(unittest.TestCase):
    ...
    def test_uses_separate_sqlite_not_analytics(self) -> None:
        from kindly_web_search_mcp_server.cache.page_sqlite import PageSQLiteCache

        with tempfile.TemporaryDirectory() as tmp:
            page_path = str(Path(tmp) / "page_cache.sqlite")
            analytics_path = Path(tmp) / "analytics.duckdb"

            cache = PageSQLiteCache(db_path=page_path)
            cache.store("https://sep.test/url", "sep content", "test")

            # Analytics path must not be created by the page cache.
