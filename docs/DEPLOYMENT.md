...
| Path | Purpose |
|------|---------|
| `/app/lancedb_data` | Semantic cache persistence (set `LANCEDB_DIR`) |
| `/app/.env` | Environment file (alternative to `-e` flags) |

### Container Security Notes

- Runs as non-root user (`app:10001`)
- Chrome sandbox disabled (`NODRIVER_SANDBOX=0`) for container compatibility
...
- Disabling universal HTML extraction (`BROWSER_EXECUTABLE_PATH=""`)
...
| `GEMINI_API_KEY` | Gemini API key (also enables `gemini_search` tool) |
...
|----------|---------|--------|
| `SEMANTIC_CACHE_ENABLED` | `true` | Enable LanceDB semantic cache |
| `QUERY_UNDERSTANDING_JSONL_ENABLED` | `true` | Capture query-understanding training JSONL |
| `RERANKING_ENABLED` | `true` | Enable Jina cross-encoder reranking |
...
|----------|---------|-------|
| `RATE_LIMIT_WEB_SEARCH_RPS` | `4.0` | Cheap tools (web_search, get_content, gemini_search) |
| `RATE_LIMIT_WEB_SEARCH_BURST` | `12` | Burst capacity for cheap tools |
| `RATE_LIMIT_EXPENSIVE_RPS` | `0.5` | perplexity_search only |
| `RATE_LIMIT_EXPENSIVE_BURST` | `1` | Burst for perplexity |
...
For horizontal scaling, use a shared volume for `LANCEDB_DIR` or consider disabling semantic cache per-instance.

### Browser Pool Sizing

| Variable | Default | Scaling Notes |
|----------|---------|---------------|
| `NODRIVER_BROWSER_POOL_SIZE` | `1` | Increase for high concurrency |
| `NODRIVER_REUSE_BROWSER` | `1` | Keep enabled for efficiency |
| `NODRIVER_ACQUIRE_TIMEOUT_SECONDS` | `30` | Increase if pool contention |
...
|----------|---------|-------|
| `WEB_SEARCH_MAX_CONCURRENCY` | `3` | Clamped 1-5 |
...
| `DIAGNOSTICS` | `0` | Enable verbose diagnostics (set to `1`) |
| `STRUCTURED_LOGGING` | `false` | JSON structured logs for Loki |
...
Disabled in container environments (`NODRIVER_SANDBOX=0`). This is standard practice for:
- Docker containers
- WSL environments
...
| Browser not detected | Set `BROWSER_EXECUTABLE_PATH` explicitly |
| LanceDB write errors | Ensure `LANCEDB_DIR` is writable |
...
| Timeout errors | Increase `TOOL_TOTAL_TIMEOUT_SECONDS` |
...
```bash
DIAGNOSTICS=1 LOG_LEVEL=DEBUG kindly-web-search-mcp-server start-mcp-server
```
