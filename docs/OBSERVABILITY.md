...
Local analytics can be disabled with `ANALYTICS_ENABLED=false` or redirected
with `ANALYTICS_DUCKDB_PATH`.
...
- Head-based sampling is controlled by `OTEL_SAMPLING_RATIO` (default 15%).
- Attribute values are aggressively truncated via `OBSERVABILITY_MAX_TEXT_CHARS` and `OBSERVABILITY_MAX_ITEMS` to keep cardinality low.
...
**Setup (use standard LANGFUSE_* or LANGFUSE_* convenience):**
```powershell
...
- `settings.py` — All `OTEL_*`, `GRAFANA_CLOUD_*`, and `LANGFUSE_*` configuration
