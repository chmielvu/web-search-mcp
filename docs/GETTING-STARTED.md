...
# Or additional API-backed providers:
...
**Requirements:** Set `COMPOSIO_API_KEY` and `COMPOSIO_USER_ID` environment variables.
...
export TOOL_TOTAL_TIMEOUT_MAX_SECONDS=600
```
...
**Fix:**
1. Increase retries: `NODRIVER_RETRY_ATTEMPTS=5`
2. Ensure proxy settings don't interfere: `NO_PROXY=localhost,127.0.0.1`
3. Increase ready timeout: `NODRIVER_DEVTOOLS_READY_TIMEOUT_SECONDS=20`
...
```bash
export YOUTUBE_TRANSCRIPT_PROXY_URL="http://your-proxy:8080"
```
...
See CONFIGURATION.md for the full list of `AGENTIC_RESEARCH_*` variables and `API.md` for the exact MCP tool contract.
