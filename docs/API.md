...
- Requires at least one configured search provider: `SEARXNG_BASE_URL`, `TAVILY_API_KEY`, `BRAVE_API_KEY`, `JINA_API_KEY`, or `GEMINI_API_KEY`.
...
- Semantic cache enabled by default (`SEMANTIC_CACHE_ENABLED=true`).
...
| `char_length` | `int` | No | `20000` | Maximum characters to return. Clamped to `GET_CONTENT_MAX_CHARS` (default 50000). |
...
- Total tool timeout: `TOOL_TOTAL_TIMEOUT_SECONDS` (default 120s).
...
**Notes:**
- Requires `NANOGPT_API_KEY` for the primary NanoGPT model chain. When `GEMINI_API_KEY` is set, the final model fallback is `gemini-3.5-flash` through the Gemini API.
...
- Requires `COMPOSIO_API_KEY` + `COMPOSIO_USER_ID` environment variables.
...
- Cloud IPs (AWS/GCP/Azure) may be blocked; use `YOUTUBE_TRANSCRIPT_PROXY_URL`.
- Max transcript length: `YOUTUBE_TRANSCRIPT_MAX_CHARS` (default 50000).
...
| `timeout` | Request exceeded `TOOL_TOTAL_TIMEOUT_SECONDS`. |
...
|------|---------------------|
| `web_search` | `SEARXNG_BASE_URL` or `TAVILY_API_KEY` or `BRAVE_API_KEY` or `JINA_API_KEY` or `GEMINI_API_KEY` |
| `gemini_search` | `GEMINI_API_KEY` |
| `perplexity_search` | `POLLINATIONS_API_KEY` |
| `quick_web_search` | `COMPOSIO_API_KEY` + `COMPOSIO_USER_ID` |
| `composio_similarlinks` | `COMPOSIO_API_KEY` + `COMPOSIO_USER_ID` |
| `youtube_search` | `SEARXNG_BASE_URL` |
| `youtube_transcript` | None (optional: `YOUTUBE_TRANSCRIPT_PROXY_URL` for cloud IPs) |
...
| Tool Tier | RPS | Burst |
|-----------|-----|-------|
| Lightweight (`web_search`, `get_content`, `gemini_search`) | `RATE_LIMIT_CHEAP_RPS` (default 10) | `RATE_LIMIT_CHEAP_BURST` (default 20) |
| Expensive (`perplexity_search`) | `RATE_LIMIT_EXPENSIVE_RPS` (default 2) | `RATE_LIMIT_EXPENSIVE_BURST` (default 4) |
