...
**Intent:** split  calls to paid SERP providers (Brave, Google CSE) to avoid quota exhaustion 
...
**Fix:**  The semaphore should be a module-level constant or env-configurable. (first call goes to Brave, second to CSE, third to Brave etc. - NEEDS TO BE EXTENSIBLE IN CASE other paid SERPs are added - there should be a 'serp_semaphore) tag that is set on intent level -> for evetry intent it could be different) 
