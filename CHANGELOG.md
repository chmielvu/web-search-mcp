...
- `prompts/adaptive_search.py`: `ShortQuery` (400) / `ShortPhrase` (300) schema caps now enforce the prompt checklists (proven: 401-char query, empty why, finish+queries all rejected); evidence-field docs aligned to actual keys (`evidence_consensus`, `query_integrities`, `wave_provider_calls`).
- `search/outcomes.py`: `adaptive_search` payload gains `prompt_version` (`ADAPTIVE_SEARCH_PROMPT_VERSION`), wiring the previously exported-but-unread constant into telemetry.
- `inference/catalog.py`: `adaptive_search_llm` tries `gemini-3.5-flash-lite` on both Gemini keys (`@google`, then `@google:second`) before stepping down models; new `gemini-3.5-flash-lite@google:second` provider registered.
- `search/providers/gemma_serp.py`: `_parse_response` takes the shaped query and stamps it on the `EngineCall` (was hardcoded `""`), consistent with every other provider. `search/ranking.py`: determinism comment corrected (cross-branch order follows retrieval completion), new `zero_hit_lists_dropped` counter alongside `duplicate_lists_dropped`.

### Changed — Adaptive search: dedicated high-context chain, full-weight RRF, evidence v2 (2026-09-19)
- New `adaptive_search_llm` inference chain (Gemini-first: `gemini-3.5-flash-lite@google` → `gemini-3.1-flash-lite@google:second` → `gemini-2.5-flash@google` → `gpt-oss-120b@vercel` terminal fallback, no Groq entries) with `build_adaptive_router()`; both decision stages moved off the Groq worker chain so the ranked slate with long passages no longer contends with the ~7k TPM window. Decision timeout 20s → 60s; proposal stage uses `reasoning_effort="low"` for gap analysis.
- Decision feedback is now high-context evidence v2 under a 100k-token budget (`_ADAPTIVE_FEEDBACK_BUDGET_CHARS = 400k` chars, hard-truncated with priority slate > executed set > signals): 2000-char decision passages (was 400), 10 overflow rows with passages/stage/source (was 5 title-only), `evidence_consensus`/`freshness_signal`/`final_score` replacing the raw cross-encoder float, and `overlap_rate`/`duplicate_lists_dropped`/rerank provider+model in `pool_counts` so the stopping judge can tell convergence from thin overlap.
- RRF now votes full provider weight per distinct list (RAG-Fusion consensus): the weight-splitting division is removed and adaptive `FOLLOWUP` lists carry a wave-role discount (`RRF_FOLLOWUP_WEIGHT_FACTOR`, default 0.5, validated in `(0, 1]`) so targeted waves add recall without diluting the broad wave.
- RRF duplicate guard now keys on the shaped `EngineCall.query` text (what the provider received after role shaping, stamped by `_finalize_call`) instead of the planner branch text: collapsed rewrites share one vote, distinctly shaped requests vote independently (`_rrf_list_key` + `_normalize_shaped_query`, empty-query fallback to branch text). Proven with a throwaway script (key unit cases + end-to-end `rank_and_finalize`: shared-URL consensus wins, collapsed pair drops one list).


### Changed — Brave provider consolidation (2026-09-19)
- Merged `search/providers/brave_common.py` into `search/providers/brave.py`: error classes, Brave endpoint URLs, standard-key resolution, auth headers, query bounding, and freshness translation now live directly beside the Brave LLM Context and Autosuggest provider functions.
- Deleted `search/providers/brave_common.py` with no compatibility shim (clean cutover; `brave.py` was its sole importer).

### Added — Bounded adaptive `web_search` (result-conditioned waves + synthesis) (2026-09-18)
- `web_search` is now one bounded, result-conditioned operation (`search/adaptive.py::run_adaptive_search`): the six-branch broad first fanout is preserved unchanged, then an LLM proposes 1-2 targeted follow-up queries from the ranked evidence (`prompts/adaptive_search.py`, worker chain, strict `FollowupBatch`), wave two dispatches those exact queries to the first-encounter union of wave-one providers with their own Qdrant embedding (`embedding_task=None`), and a second LLM chooses between finishing and one final targeted wave. Hard cap `MAX_SEARCH_WAVES = 3`; the final wave flows directly to synthesis with no further decision call.
- After every wave, `rank_and_finalize` re-ranks the FULL accumulated outcome tuple under the original query/research goal, so the last wave's cumulative ranking is the final global ranking. The function is now repeat-call safe (clears its own `overflow_ranked`/`candidate_embeddings`/`rerank_metadata` state, replaces `rerank_stage_summaries`, accumulates `phase_timings["search.rank"]` and the OR of `retrieve_budget_exceeded`); final indexing and terminal persistence happen once, outside the wave loop.
- `retrieve_branches` takes an explicit `branches` argument, appends wave outcomes to the live `SearchRun`, and records a required global `branch_index` in query-transform rows, provider-call rows, provider-result IDs, and integrity rows. `persist_search_outcome` joins query transforms by that explicit index (the role-to-index dictionary is deleted — multiple follow-up branches share one role) and adds `adaptive_rounds`, `adaptive_search` (`rounds`/`stop_reason`), and `synthesis` to `search_runs.payload_json`.
- Public envelope: `WebSearchPublicResponse` gains optional `synthesis` (snippet-grounded overview with inline `[cN]` citations, validated against the final slate) and `search` (`rounds` 1-3, `stop_reason`) populated by `to_public_web_search`/`to_public_web_search_from_run` for every fresh search; cursor pages emit neither field and never re-search. Tool description and CLI `search web` help describe the wave behavior; catalog version is 4.0 and `_TOOL_TIMEOUTS["web_search"]` is `None` so a three-wave run is not killed by the one-pass tool-wide timeout. `rewrite` disables only the first wave's planner rewrite.
- Degraded paths preserve ranked evidence with explicit `adaptive_search` warnings: decision failure → `decision_failed` (synthesis still attempted when evidence exists), synthesis failure → `synthesis_failed`, no final hits → `no_results` without a synthesis call, unassigned/unresponsive providers → `retrieval_failure` with no compensating rewrites. Diagnostics gain `adaptive_rounds` wave records; `_branches_from_run` enumerates accumulated outcomes instead of zip-truncating follow-up branches.
- `LLMRouter.complete_json` additionally accepts a plain JSON-schema mapping as `response_model` (the Google genai adapter's `response_json_schema` shape); the synthesis call passes `SynthesisDraft.model_json_schema()` and validates the generated JSON with the production schema.
- Live verification (frozen tests untouched): two-wave `sufficient_evidence` runs via CLI, in-process, and MCP (`tools/list` + `tools/call` on streamable HTTP, version 4.0 meta); `no_new_queries` and `retrieval_failure` (all providers disabled) terminal paths; `decision_failed`/`synthesis_failed` degraded runs preserving ranked evidence; cursor pages returning title/url-only leftovers with neither new field; `--no-rewrite` plus date/region/undated filters propagating through the follow-up wave; persisted runs show global branch indices 0-7 with follow-up transform rows joined by index. The third-wave ceiling path is code-verified (continuation only after wave two) but was not live-observed — the stopping model legitimately chose to finish; the stopping prompt was not weakened to force it.

### Changed — FastMCP 4 + SEP-2663 background tasks (2026-09-18)
- Pinned `fastmcp[tasks]>=4,<5` with `mcp>=2,<3` and `pydantic>=2.12`; dropped the unused `pydocket` direct pin. `TasksExtension` registers at server bootstrap against `FASTMCP_DOCKET_URL` (unreachable Redis probes fall back to `memory://`).
- Relocated `deep_research` to `tools/deep_research.py` with no shim. Catalog advertises `TaskConfig(mode="optional", poll_interval=30s)`. Progress uses FastMCP `Progress` (`set_total` / `set_message` / `increment`); `ctx.info` stays on foreground/legacy calls only.
- HTTP transport now matches FastMCP 4: `--http` / `--streamable-http` run `mcp.run(transport="http", host=..., port=...)`. Session identity fail-opens when `session_id`/`client_id` are absent so sessionless-era clients do not share throttle buckets.

### Removed — `youtube/` package dissolved into content/search/utils (2026-09-18)
- Transcript acquisition now lives in `content/youtube_transcripts.py` (vendor parsers, single validation/normalization pass, cache-first cascade, segment rendering); YouTube URL identity moved to `utils/youtube_urls.py`; Data API channel/uploads enumeration plus its quota tracker moved into `search/quick/quick_web_search_youtube.py`.
- `youtube/analysis.py` (GLiNER2) deleted: the `youtube_transcript` tool no longer runs entity analysis (imports, progress, markdown block, response/observability fields removed; `YouTubeTranscriptAnalysis` and the response `analysis` field pruned from `models.py`). `youtube/quality.py`, `youtube/transcript.py`, `youtube/cascade.py`, `youtube/models.py`, `youtube/url_parser.py`, `youtube/channel_api.py`, and `youtube/api_quota.py` deleted after their moves.
- Quality diagnostics are now computed inline in `tools/youtube.py` (`_truncate_segments` plus segment counts over normalized output); the content YouTube resolver reuses shared timestamp rendering instead of its own loop.

### Changed — YouTube transcript overhaul: Apify-first scraper cascade (2026-09-18)
- Transcript fetching now runs on vendor egress only: Apify `APIFY_YOUTUBE_ACTOR` (default `supreme_coder~youtube-transcript-scraper`) is the primary tier and the Bright Data YouTube Scraper API dataset (`BRIGHTDATA_YOUTUBE_DATASET_ID`, default `gd_lk56epmy2i5g7lzu0k`) is the fallback. Backends are `auto|apify|brightdata` (`YOUTUBE_TRANSCRIPT_BACKEND`); the cascade and cache entry points are now async (`fetch_transcript_cascade`, `fetch_transcript_with_cache`).
- New reusable modules: `content/brightdata_scraper.py` (`BrightDataScraperClient`, `ScraperClientError`, `get_brightdata_scraper_client` for `POST /datasets/v3/scrape`) and `content/youtube_transcripts.py` (`apify_fetch_transcript`, `brightdata_fetch_transcript`, `ScraperTranscriptError`, segment parsers). The existing `content/remote_clients.py` Apify run-sync client is reused unchanged.
- Removed datacenter-IP tiers with no backward compat: deleted `youtube/yt_dlp_backend.py` and `youtube/whisper.py` (yt-dlp, Cloudflare/HF Whisper, cobalt audio, legacy youtube-transcript-api fetch); dropped `YOUTUBE_TRANSCRIPT_PROXY_URL`, `WHISPER_SPACE_URL/ID/TIMEOUT`, `COBALT_BASE_URL/TIMEOUT`, `CLOUDFLARE_*`/`CF_WHISPER_*` settings; scrubbed proxy guidance from `errors.py`; CLI `--backend` help now lists `auto, apify, brightdata`.
- The content YouTube resolver and the `youtube_transcript` tool no longer touch yt-dlp metadata (title fields now report `None`, language falls back to the request or `und`); `youtube/RESEARCH.md` moved to `docs/youtube-scraper-research.md`; package guidance refreshed in `content/AGENTS.md`.
...
- **Hot-Path Performance**: Precompiled static regular expressions at module scope in `index/bm25_encoder.py`, `content/machine_readable.py`, and `tools/code_search/grepapp.py`; precomputed normalized scopes in `tools/code_search/filters.py` to eliminate per-hit set reallocations; refactored `search/postprocess.py::apply_domain_boost` to single-pass partitioning and moved imports to module top level.
- **Error Diagnostics**: Standardized on `{identifier!r}` formatting across resolver error messages (`arxiv.py`, `telegram.py`, `wikipedia.py`, `stackexchange.py`, `jobs.py`); narrowed broad `except Exception` blocks in `utils/duckdb_log_handler.py` and `content/machine_readable.py`.
### Fixed — Sprint Review Quick Wins (2026-09-16)
- **Shutdown logging**: `BatchDuckDBLogHandler.close()` now flushes pending records before marking the handler closed; the previous order discarded buffered records below capacity during shutdown.
- **Strict token usage parsing**: bool and non-integer-only token payloads now produce no usage object, valid counts still derive totals, and total-only `LLMUsage` values are retained in summary payloads.
- **Cache isolation**: rewrite-cache metadata is copied at insertion so per-run branch and graph annotations cannot leak into later cache hits.
- **Defensive boundaries**: hydration, Twitter, and embedding paths now use explicit runtime guards instead of production assertions.
- **Code-search hot path**: normalized language-extension sets are built once at import time instead of being reallocated for every hit.
- **Package guidance**: corrected the stale utility-module inventory and documented the code-search cache, process-log sinks, and usage helper in their nearest package guidance.
- **Analytics schema rollover**: embedding migrations now drop every cataloged index before renaming legacy vector tables, retain `run_key`/`title` in fresh 384-dimensional tables, and rebuild the source-managed `vw_embedding_similarity` view.

### Changed — Blocklist store relocated under `duckdb_data/` (2026-09-16)
- **The blocklist was only found when the process working directory was the repository root.** `search/blocklist.py` resolved its store through `getattr(settings, "blocklist_sqlite_path", None)` with a fallback to `Path("data") / "blocklist.sqlite"`, but no such settings field existed — `getattr` always returned `None`, so the CWD-relative fallback was the only path ever used. A caller started from another directory (the CLI from an arbitrary shell, a harness that spawns the server elsewhere) would have created an empty store instead of reading the populated one, silently filtering nothing out of `web_search`.
- The store now lives at `duckdb_data/blocklist/blocklist.sqlite` next to `blocklist_seed_farms.txt`. `utils/paths.py` gained `BLOCKLIST_DIR` and `DEFAULT_BLOCKLIST_DB` alongside the other repository-anchored databases, `ensure_duckdb_dirs()` creates the directory, and `settings.blocklist_sqlite_path` (override: `BLOCKLIST_SQLITE_PATH`) reads the new default. `_resolve_db_path()` uses the setting directly and falls back to the anchored constant rather than a relative path — an empty override still resolves to the repository copy instead of `Path("")`.
- `duckdb_data/AGENTS.md` inventory corrected (it still named `data/blocklist.duckdb`), the dead `data/blocklist.duckdb` and `/data/` ignore entries removed, and `search/AGENTS.md` no longer describes the store as DuckDB-backed.
- **Verification:** the 80,990,208-byte store moved with an unchanged `sha256` (`df2c215b…`). A smoke run from a temp working directory resolved `duckdb_data\blocklist\blocklist.sqlite`, loaded 410,871 active rows (ssss 408,926 / huge-ai 1,535 / bad-websites 389 / farm-seed-local 21) into an index of 410,245 sub-hosts, 10 exact hosts and 1 complex regex, blocked a known-blocked host, passed a control URL, and had `filter_blocked_results` drop the blocked row while keeping the clean one. `ruff check src/`, `ruff format --check src/` (381 files) and `ty check src` are clean, and `web-search-cli doctor` exits 0.
...
- **The analytics dashboard could not render its KPI strip.** `analytics/ui.py` passed `trend=Rx("…_trend")` into `Metric`, but Prefab's `trend` field is `Literal["up", "down", "neutral"] | None` — unlike `value` and `delta`, it takes no reactive binding, so pydantic raised `ValidationError` while building the app. The direction is now derived from the same delta the card shows (`_trend(data.get("…_delta"))`), and `build_app_ui` was probed with sample data to confirm it returns a `PrefabApp`.
- Progress: `uv run --no-sync ty check src` reports **182 → 0** diagnostics, and the CI step is now blocking. The remainder is dominated by library-stub mismatches (telethon, `duckdb.connect`, qdrant-client, ddgs, FastMCP `Context`, OTel `SimpleQueue`/`AbstractSet`) and repeated `invalid-argument-type` clusters in `tools/code_search/*`; CI stays advisory until the count reaches zero.
- Later in the same pass the remaining warnings were cleared too: a third silently-discarded argument (`gemma_serp.py` passed `diagnostics=` to `WebSearchResult`, which has no such field, so the Pollinations grounding evidence never reached anything — that intent needs a schema decision), two dead conditions (`SpanContext` defines no `__bool__`; `asyncio.gather` over one task cannot be empty), a deprecated `asyncio.iscoroutinefunction` call, eight stale blanketed `# type: ignore` comments that were hiding two genuinely optional `pymupdf` imports (now in `allowed-unresolved-imports` where optional extras belong), and a telethon call that could pass a `None` username plus a union-typed message count.
- **Verification:** `ruff check src/` clean, `ruff format --check src/` clean, `ty check src` clean, 364 modules import with none failing, `cli --help` exits 0, the cache-restore path round-trips unchanged, and the wildcard matcher was probed directly (5/5 cases correct, 3 of which failed before the fix).
