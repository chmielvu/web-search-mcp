...
- Progress: `uv run --no-sync ty check src` reports **172 → 48** diagnostics. The remainder is dominated by library-stub mismatches (telethon, `duckdb.connect`, qdrant-client, ddgs, FastMCP `Context`, OTel `SimpleQueue`/`AbstractSet`) and repeated `invalid-argument-type` clusters in `tools/code_search/*`; CI stays advisory until the count reaches zero.
