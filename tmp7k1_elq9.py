...
import faulthandler, asyncio, time, sys, uuid, httpx
...
async def test_plan_search(query: str, research_goal: str) -> str:
    ...
    print(f'[test] plan_search starting...', file=sys.stderr, flush=True)
    t0 = time.monotonic()
    try:
        plan = await asyncio.wait_for(plan_search(run), timeout=25)
        print(f'[test] plan_search done in {time.monotonic()-t0:.1f}s', file=sys.stderr, flush=True)
        return f'ok'
    except asyncio.TimeoutError:
        print(f'[test] TIMEOUT', file=sys.stderr, flush=True)
        raise
