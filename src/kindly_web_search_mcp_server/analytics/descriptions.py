...
_OBJECT_DESCRIPTIONS: dict[str, str] = {
    ...
    "adaptive_search_runs": (
        "Adaptive web-search metadata normalized from search_runs payloads: round count, "
        "stop reason, prompt version, and synthesis text."
    ),
    "adaptive_search_rounds": (
        "One row per adaptive controller round, preserving its branch range, query list, "
        "candidate/domain counts, provider failures, decision, and reason."
    ),
    "adaptive_search_proposals": (
        "Runtime adaptive TargetedQuery proposals, including normalized dispatch acceptance "
        "and the branch assigned to accepted proposals."
    ),
    # Derived views
    "vw_events": (
    ...
    "vw_adaptive_search_run": (
        "One row per adaptive web-search run, joining normalized adaptive metadata to the "
        "canonical search run and adaptive LLM call log."
    ),
    "vw_adaptive_search_round": (
        "One row per adaptive search round with its recorded controller metrics and persisted "
        "branch, provider-call, and provider-result counts."
    ),
    "vw_adaptive_followup_yield": (
        "Accepted adaptive follow-up branches with existing branch-contribution metrics, "
        "provider-call status counts, and query-variant execution state."
    ),
    "vw_adaptive_output_followthrough": (
        "Adaptive web-search output items linked to the existing search-to-fetch follow-through view."
    ),
    "vw_adaptive_search_end_to_end": (
        "One analyst-facing adaptive run rollup combining round progression, follow-up yield, "
        "output items, and content-fetch follow-through."
    ),
