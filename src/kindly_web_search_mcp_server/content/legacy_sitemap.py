"""Legacy semantic sitemap generation using Crawl4AI remote deep crawl."""
...
class LegacySitemapConfig:
    """Configuration for legacy semantic sitemap generation."""
...
async def crawl_and_extract_pages(
    url: str,
    *,
    config: LegacySitemapConfig,
) -> dict[str, Any]:
    """Crawl a site via Crawl4AI deep crawl and extract heading sections."""
    client = get_crawl4ai_client()
    if client is None:
        raise RuntimeError(
            "CRAWL4AI_BASE_URL not configured. The legacy sitemap fallback requires a remote Crawl4AI server."
        )
