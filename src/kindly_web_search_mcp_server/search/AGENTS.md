...
- Merge uses weighted RRF over per-`(adapter, shaped-request)` ranked lists exactly as providers returned them. The duplicate guard keys on the shaped `EngineCall.query` text (what the provider received after role shaping), not the planner branch text: collapsed rewrites share one vote, distinctly shaped requests vote independently. Every surviving list votes its full provider weight so cross-query agreement accumulates (RAG-Fusion consensus). Adaptive `FOLLOWUP` lists carry a wave-role discount (`RRF_FOLLOWUP_WEIGHT_FACTOR`, default 0.5) so targeted waves add recall without diluting the broad wave. Weights: `settings.rrf_provider_weights` + `rrf_bm25_weight`. `provider_consensus_rrf_score` is deleted.
