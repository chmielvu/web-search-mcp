# 2026-09-09 code_fetch single-file fast lane and routing guidance
- **Scope**: User clarified that an explicit file-like `path` (`*.md`, `*.py`, and similar) must select the fast lane rather than relying on a broad path/nonblank heuristic.
- **Shipped**: `tools/code_search/exploration.py` now treats an explicit file-like `path` as the direct GitHub hydration signal; redundant file filters do not initialize the snapshot manager. File-like query forms still normalize to the same path. `middleware/query_guidance.py` now routes contents-only follow-ups to `fetch` and repository intelligence to `code_fetch` query/symbol continuations. Server instructions, `docs://workflow`, and `tools/AGENTS.md` teach the same boundary.
- **Verification**: Throwaway smoke proved `code_fetch(path="README.md", language="markdown")` returns hydrated content without calling `get_snapshot_manager`; path/query classification and next-tool guidance passed. Ruff, format, and AST checks passed on the changed Python files. Tests were not run because the repository test suite is frozen.
- **Known**: The direct path contract intentionally remains a read operation when `query`, `symbol`, tree depth, or cursor pagination is supplied; those requests retain repository semantics. GitNexus change detection remains over-broad because the working tree contains unrelated pre-existing edits.

# 2026-09-09 utils/ regex & heuristics audit implementation + Jina live exercise
- **Scope**: User directive: implement fixes from the 2026-09-09 read-only audit of utils/ regex/heuristics; remove dead code/shims; merge util files where possible. Followed by live Jina exercise (11 sites, production headers) that surfaced three additional text_clean findings (F1-F3), all fixed here.
- **Shipped (utils/)**: query_pipeline (P0 non-overlap union `_CLASS_PRECEDENCE` + post-splice protected-range re-derivation + NOT collapse + `_BARE_LANG_WORDS` whitelist + `c++`/`c#` lang capture); query_understanding (TIME_RECENT interval forms beat bare "past"; `_PRODUCT_VS_CODE` `(?![\w-])` guard; dead `classify_intent_by_embedding` removed — zero callers); text_clean (`_JINA_FRONTMATTER_RE` capture group fixes EOF truncation of last envelope field; fence-aware `_repair_links_prose_only`; ftfy import hoisted; © footer patterns); content_classify (`_gopher_signals` line signals prose-only — fenced comments no longer junk-flag docs); text_chunking (`slice_content` zero-progress pagination guard, mirrors chunk_text); github (segment validation, `..` rejected, `#ref` stripped, identity stays owner/name); entity (version must be digit-initial).
- **Shipped (content/ + merges)**: stages.py `_fetch_via_jina` strips frontmatter envelope before chrome/classification (was shipped in artifact on 11/11 live sites); `snippet_normalizer.py` merged into public_output.py (single caller) with fixed HTML-tag shape + line/bracket-anchored chrome patterns; dead `utils/diagnostics.py` deleted (zero importers). `utils/github.py` → url_canonicalize merge SKIPPED: frozen test pins the module path and repo-identity ≠ URL canonicalization.
- **Verification**: 42/42 behavior probes green (throwaway suite, deleted post-run); ruff check+format clean on all 9 touched files; full import smoke incl. search.retrieval green. Tests NOT run (frozen per AGENTS.md). detect_changes reported CRITICAL risk but the flagged scope is mostly pre-existing uncommitted user work in the worktree (inference/catalog, search/ranking, settings, url_canonicalize, tests/test_inference_subsystem) — my footprint is the 11 files listed above; GitNexus impact on shape_for_branch/slice_content was LOW pre-edit.
- **Known**: frozen `tests/test_github_normalization.py` pins remain valid (all 5 accept cases + 5 reject cases still pass by construction); frozen `heuristics.*` test imports reference a package that no longer exists on disk — pre-existing, unrelated.
# 2026-09-08 QA batch 2 — five output-quality fixes
- **Scope**: User directive: fix U+2011 leak, session_id 100% null, freshness recalibration, DDG sitelink bundles, slug-variant dedup (all from the 2026-09-08 QA exercise).
- **Shipped**:
  - `utils/text_clean.py`: `_FANCY_QUOTES` + U+2010/2011/2012/2015/soft-hyphen folds (query ingress).
  - `utils/observability.py`: `_resolve_session_id()` (FastMCP ctx, honest None outside) + `_insert_tool_call_analytics` fallback population of `tool_calls.session_id`.
  - `search/ranking.py`: freshness boundary → `FRESHNESS_MAX_AGE_DAYS` setting (default 90); future dates clamp `fresh`.
  - `search/providers/ddg.py`: `_split_sitelink_title` — bundle titles (2+ segments joined by `...`) collapse to primary segment.
  - `utils/url_canonicalize.py`: `_fold_slug` — date-path ≡ date-slug ≡ underscore variants fold; RRF dedup fuses tianpan.co-style duplicates (verified 2 lists → 1 fused entry, providers merged).
- **Verification**: ruff+format clean on all 6 files; LSP clean (ranking/settings/observability/url_canonicalize; ddg 1 pre-existing hint); consolidated smoke asserts all five behaviors; RRF fusion proof live. `verified` for all five.
- **Known**: ddgs 9.14.2 returns only body/href/title — no date fields at all from DDG; published_date comes from exa/serper etc. Freshness QA "inconsistency" was snippet-visible updated dates, not a label bug; fix is config + clamping, documented in CHANGELOG.

# 2026-09-08 Qdrant web-results index: empty-collection root cause + write-path restore
- **Scope**: User directive: analyze Qdrant read+write paths, inspect HF Space via hf MCP, smoke-test why it returns nothing; plus chain correction (qwen3.6 → primary Groq key).
- **Root cause (write path)**: `index_final_results`/`WebResultsIndex` had ZERO production callers — no code path ever upserted into `web_results_384d`. HF Space telemetry confirmed: since the 2026-09-07 Arctic-384 rebuild, exactly 2 collection DELETEs + 1 create + 1 point-PUT (my Sep-8 test probes) and `points_count=0` at investigation time. Read path was healthy all along — seeding 3 docs made `search_qdrant` return all 3, cross-encoder scored them 0.78/0.77 (they lost the top-15 cut against 92 real-web candidates — expected funnel competition, not a bug).
- **Auth gotcha (verified)**: the HF Space docker-proxy only accepts `Authorization: Bearer`. qdrant-client `api_key=` sends `api-key:` → 404 HTML. `auth_token_provider=` → `BearerAuth` → works. Both `search/providers/qdrant.py` and `index/web_results_index.py` already used `auth_token_provider` correctly; the earlier `HF_TOKEN` auth failure hypothesis was refuted.
- **Shipped**: `search/service.py::_schedule_web_results_indexing` — fire-and-forget task after `rank_and_finalize`: upserts final 15 results via `index_final_results`, reusing rerank `candidate_embeddings`/`query_embedding` when URLs match, else one `embed_texts` batch; skips on unresolvable embeddings; failures debug-logged, never fatal; task set `_INDEX_TASKS` for GC. Gated by existing `WEB_RESULTS_INDEX_ENABLED`+`QDRANT_SPACE_URL`.
- **Verification**: ruff+format clean; LSP clean. E2E live: full pipeline run → collection 0→15 points in ~3s → `search_qdrant` returned 15 self-indexed results on the next call. Probe points (doc1-3, idx-a/b) deleted; collection back to exact production state.
- **Known**: qwen3.6 chain correction (primary key) verified via import smoke; test file's chain assertions already updated in the prior pass and still valid (all three Groq tiers → `GROQ_API_KEY`). DuckDB `candidate_embeddings`/`query_embeddings` tables have been dead since 2026-07-22 (separate observability gap, not touched here). Groq GPT-OSS 401 in smoke runs is an env key issue (`gsk_OUV9…` rejected) — rewrite fell back correctly; unrelated to Qdrant.

# 2026-09-08 degoog disabled + worker_llm Qwen rewrite chain + Groq roster refresh
- **Scope**: User directive (2026-09-08, post QA exercise): (1) disable degoog provider, (2) rewrite chain = Groq GPT-OSS → Qwen 3.8 (primary key) → Qwen 3.6 (second key) → Vercel → HF, (3) remove llama models from registry (dropped from Groq roster), add qwen/qwen3.8-27b.
- **Shipped**:
  - `settings.py`: `DISABLED_PROVIDERS` default `serpapi` → `serpapi,degoog`. No new rewrite-model knobs — qwen ids live in the catalog, not env.
  - `inference/catalog.py`: `worker_llm` chain now `gpt-oss-120b@groq` → `qwen/qwen3.8-27b@groq` → `qwen/qwen3.6-27b@groq:second` → `gpt-oss-120b@vercel` → `gpt-oss-120b@huggingface`. `gpt-oss-120b@groq:second` dropped from the chain (second key now carries Qwen 3.6). Added `_register_groq_text_model("qwen/qwen3.8-27b", ...)`; deleted `llama-3.1-8b-instant` and `llama-3.3-70b-versatile` definitions+providers.
  - `tests/test_inference_subsystem.py::test_get_chain_valid` / `test_live_provider_model_entries_match_api_inventory` cut over to the new chain shape (5 entries, fallbacks=4) and roster (qwen3.8 in, both llama out). Frozen-test edit per user directive.
  - Docs: `inference/AGENTS.md` chain example + Groq inventory row; CHANGELOG `[Unreleased]` three entries.
- **Verification**: `ruff check` + `ruff format --check` clean on the 3 touched files. Import smoke green: chain resolves in order with expected model_ids, `qwen/qwen3.8-27b@groq` and `qwen/qwen3.6-27b@groq:second` resolve, llama ids absent from registry, `settings.disabled_providers == ('serpapi','degoog')`, `provider_is_reachable(degoog)` False with DEGOOG_BASE_URL set (proving the disable path, not just unconfigured). LSP clean on catalog.py/settings.py; test file has 4 pre-existing errors (stale `rerank.llm_rerank`/`rerank.providers` imports from the 2026-09 rename, lines 220/322/405-406) — untouched, unrelated.
- **Known**: frozen tests not run (per testing freeze). Analytics DB was unlocked earlier this session by terminating the running web-search-mcp server; restart the OMP session/MCP connection to restore web_search MCP tools and pick up these code changes.

# 2026-09-07e A/B runtime + evals runner deleted; langfuse no-op removed; memoize renamed
- **Scope**: User directive from recommendation list: (1) delete `evals/judges._send_to_langfuse`, (2) rename `_memoize_canonicalize` → `memoize_canonicalize` in merge.py, (3) delete the ab_testing runtime + ab_* DuckDB tables + AB_TESTING_ENABLED settings, (4) delete evals/runner.py + the mcpevals extra.
- **Deleted**: `analytics/ab_testing/` (5 modules + AGENTS.md), `writers/ab_schema.py`, `cli/commands/experiments.py`, `analytics/evals/runner.py`, 7 frozen test files (`test_ab_integration/models/settings/wiring_qu/schema/views/shadow_runner`). DDL purge: ab table-name constants, `_ensure_ab_*` + `_AB*_TABLE_NAME` + `insert_ab_*` from writers/inserts/core/__init__, schema.py's `from .ab_schema import` re-import, duckdb_store explicit re-export + `__all__`, views `_build_ab_view_sql` + `v_ab_*` view creation + ab imports. Settings purge: `ab_testing_enabled`/`ab_config_path`/`ab_shadow_mode_default`/`ab_assignment_cache_ttl_seconds` + unused `DEFAULT_EXPERIMENTS_YAML` import; `EXPERIMENTS_DIR` + `DEFAULT_EXPERIMENTS_YAML` from utils/paths.py. pyproject: `eval = ["mcpevals"]` extra + stale `tests/test_shadow_runner.py` per-file-ignores entry.
- **Rename**: `merge._memoize_canonicalize` → `memoize_canonicalize` (callers: reciprocal_rank_fusion internal, ranking.py import/call site, test_search_merge_cache.py, test_search_ranking.py docstring).
- **Removed**: `evals/judges._send_to_langfuse` + its call inside `_persist_judge_call` (langfuse not a dependency — function was a guaranteed silent no-op).
- **Docs**: analytics/AGENTS.md (ab_testing row removed, evals row updated, views row de-A/B'd), cli/AGENTS.md (experiments.py + command removed from tree), skills/web-search-cli/SKILL.md (experiments section/command/env row removed), CHANGELOG.
- **Verification**: ruff check tests/ clean; src/ has only the pre-existing orchestrator.py:314 F821; import smoke green for 11 modules incl. settings/cli.app/planning; runtime assertions — no `_ensure_ab_experiments`/`insert_ab_experiment`/`ABE_TABLE_NAME` on duckdb_store, no `ab_testing_enabled`/`ab_config_path` on settings, `memoize_canonicalize` importable. `uv run` full sync was blocked by a running web-search-mcp.exe process holding the venv (os error 32) — used `--no-sync` throughout; pyproject validated via tomllib.
- **Watch**: the two stale-anchor remaps during multi-cut sequences (duckdb_store code-search imports, SKILL.md continuation line) were caught by ruff/LSP and repaired in-pass — re-verify with ruff after any future multi-hunk cut series.

# 2026-09-07d provider_catalog merged into provider_registry
- **Scope**: User directive: provider_catalog + provider_registry naming is confusing, merge them.
- **Audit**: catalog (220 ln) = ProviderDefinition model + _definition helper + brightdata_provider_call_timeout_seconds + PROVIDER_DEFINITIONS_LIST (16-provider matrix, adapter module/function pairs, env-var all_of/any_of, resilience metadata); registry (196 ln) = ProviderAdapter protocol + _make_adapter eager-resolution wiring + PROVIDER_DEFINITIONS/PROVIDER_ADAPTERS MappingProxyTypes + reachability + RR selection (google-paid, semantic-tavily) + diagnose_providers. Data/behavior halves of one concept; 3 src consumers (planning, retrieval, providers/base) already import only the registry.
- **Shipped**: catalog content folded into provider_registry.py (model + helper + list before PROVIDER_DEFINITIONS mapping); provider_catalog.py deleted via git rm; merged __all__ carries all 15 public names. Cutover: 8 test files (ast_edit absolute-import rewrite, 8 replacements), providers/__init__.py + search/__init__.py docstrings, providers/base.py comment, tests/test_provider_resilience.py module-list docstring.
- **Verification**: ruff clean on all touched files; import smoke green; merged-surface assertions — 16 definitions, keys == adapter keys, qdrant.requires_embedding, brightdata timeout = retrieve budget; functional — select_paid_google_provider round-robin alternates, diagnose_providers returns 16 rows. Frozen tests not run. Pre-existing Pyright noise in test_provider_resilience.py (dict invariance) and test_provider_registry.py (Literal 15 vs 16) unchanged — not from this merge.

# 2026-09-07c search/ shim purge + intents merge
- **Scope**: User directive: candidates in search/ — normalize.py + entity_extractor.py shims; intents.py + intent_policy.py merge; possible merge of postprocess/graph_expansion/keyword_extract/blocklist/understanding.
- **Deleted**: `search/normalize.py` (pure alias for `utils.text_clean.clean_query`; 9 importers re-pointed incl. providers/brave.py and understanding/resolver.py found only via import-smoke failure), `search/entity_extractor.py` (pass-through; `content/fetch_pipeline.py:220` now calls `get_gliner_client().extract_entities()` directly), `search/understanding/schema.py` (zero importers — JSON-schema orphan; the /classify endpoint never receives it).
- **Merged**: `intent_policy.py` → `intents.py` (sole production consumer `planning.py`; test's `monkeypatch.setattr(intent_policy_module, "settings", ...)` re-pointed to the merged module — behavior preserved since `resolve_intent_policy` reads module-global `settings`).
- **Fixed latent bug**: `scripts/live_web_search_quality_report.py` + `scripts/rerank_eval_capture.py` imported `canonicalize_url` from `search.normalize`, which never exported it (AttributeError at HEAD if ever run); re-pointed to `utils/url_canonicalize.py`.
- **Kept separate** (caller-verified rationale): blocklist.py (self-contained SQLite subsystem; scripts/seed_blocklist.py + rerank_eval_capture.py), graph_expansion.py (analytics/graph_replay.py consumes `GraphExpansionDecision`), keyword_extract.py (YAKE dependency isolate), postprocess.py (apply_domain_boost consumed by ranking.py + test), understanding/ (adapter 430ln → ml/gliner_client + youtube/analysis; resolver 115ln → planning; models → contracts + analytics/training + utils/query_pipeline; merging would couple 3 unrelated consumer groups).
- **Docs**: search/AGENTS.md Key Files (intents row merged, keyword_extract corrected from "Rake/Keybert" to YAKE, `entity/gliner_client.py` → `ml/gliner_client.py`), CHANGELOG.
- **Verification**: ruff clean on all touched files; import smoke green (planning, options, graph_expansion, understanding.*, fetch_pipeline, academic, graph_replay, gliner_client, youtube.analysis); functional smoke — normalize_intent aliases/defaults, resolve_intent_policy news freshness+exa args, goggles-not-in-general, apply_search_options roundtrip. Frozen tests not run.

# 2026-09-07b Analytics dead-code purge + observability/ absorption
- **Scope**: User directive: analyze analytics/ (and observability/) for dead code, legacy shims, merge/helper opportunities; also audit judge-related files for shims/duplicates.
- **Deleted** (all caller-verified zero): `analytics/observability_rows.py` (8-line tombstone), `observability_tables.py` + `observability_store.py` (pure re-export shims), `observability_schema.py` (single-function ensure shim consumed only by observability_inserts), `observability_inserts.py` (`insert_provider_health_transition` zero callers), `summaries.py` (`refresh_summary_tables` zero callers — summary tables still ensured at bootstrap but never refreshed), `feedback.py` (drift/SLO/regression/report zero callers; `compute_ndcg_at_10` only test-consumed), `tests/test_feedback_ndcg_fixture.py`, top-level `observability/` package (events.py absorbed; AGENTS.md deleted).
- **Renamed**: `analytics/observability_ids.py` → `ids.py`; cutover: feedback_labels.py, graph_feedback.py, rerank/observability.py, search/outcomes.py, search/planning.py, search/retrieval.py, utils/observability.py, tests/test_feedback_labels.py.
- **Cycle fix**: `utils/observability.py` now imports `PERSISTED_EVENT_PREFIXES` lazily inside `_persist_analytics_event` — `analytics/__init__` eagerly imports duckdb_store → writers → telemetry → utils.observability, so a top-level import of `analytics.events` from utils.observability is circular.
- **Re-export hygiene**: removed dead `_ensure_provider_health_transitions` from `duckdb_store`/`writers/__init__` `__all__`s (zero consumers; DDL stays in `writers/schema.py::_ensure_store_schema`).
- **Judge audit (kept, distinct paths)**: `judges.py` FlockMTL 6-facet → `llm_judgments`; `search_relevance_judge.py`+`judge_prompt.py`+`judge_runner.py` live 4-D judge → `judge_evaluations` (via `judge_runner`, wired into `tools/ai_search.py` gemini/grok paths + `utils/background_tasks.py`); `evals/judges.py` offline strict-JSON → `eval_judge_calls`/`eval_scores` (consumed by `evals/runner.py`, which itself has no src callers — mcpevals-extra-only, flagged previously). Not duplicates: different models, prompts, tables, sync/async models.
- **Flagged (unchanged)**: `judge_prompt.parse_judge_response` is test-only dead code (`search_relevance_judge._parse_response` supersedes it; test_judge_prompt.py has a dedicated class); `evals/judges._send_to_langfuse` silently no-ops (langfuse not installed, not in pyproject); `judge_calibration.calibrate_judge` legacy 4-D Pearson path is test-only (no `known_queries` producer; the κ harness `run_calibration` is the documented loop). `judges.py`'s `from concurrent.futures.thread import _worker` is a legitimate CPython internal for daemon-worker spawning, not dead.
- **Verification**: `ruff check` clean on all touched files; import smoke green for all analytics + judge + consumer modules; `ast.parse` green on edited files. Frozen tests not run.

# 2026-09-07 training/, evals/, ab_testing/ folded into analytics/
- **Scope**: User directive: move the three sibling packages inside `analytics/`, but first audit them for dead code, redundancy, and consolidation opportunities (gitnexus-refactoring skill).
- **Audit findings (no action taken, flagged)**: `ab_testing/wiring.get_ab_overrides`, `ab_testing/shadow_runner.run_shadow`, `ab_testing/assignment.get_assigned_variant` have **zero production callers** — wired only through settings + YAML + tests (documented latent framework). `evals/runner.py` (`run_eval_case`/`run_dataset`) has no src callers — justified only by the optional `mcpevals` extra. The two judge stacks are genuinely distinct: `analytics/judges.py` = FlockMTL 6-facet in-DB judge; `evals/judges.py` = offline OpenAI-compatible strict-JSON judges with Langfuse. `cases.py` `load_eval_cases`/`save_eval_cases` aliases and `metrics.py` `evidence_rate` alias are test-consumed. GitNexus impact returned CRITICAL (120/187 impacted) for move anchors — known mis-indexing pattern; grep ground truth (4 src callers + 12 test files) used instead.
- **Shipped**: `git mv` of all three packages under `analytics/`; `analytics/evals.py` renamed → `eval_schema.py` (collides with the `evals/` package; 3 internal importers re-pointed; `analytics/tabs/evals.py` untouched); internal relative imports of moved files deepened (`..settings` → `...settings`, `..analytics.duckdb_store` → `..duckdb_store`); external cutover: `search/outcomes.py`, `search/planning.py`, `search/understanding/resolver.py`, `cli/commands/experiments.py`, `tools/code_search/models.py`, + 12 test files including 27 dynamic `patch("kindly_web_search_mcp_server.…")` string targets. Docs: analytics AGENTS.md Key Files, CHANGELOG `[Unreleased]`.
- **Verification**: import smoke — all 17 analytics modules + 7 consumer modules import OK under `uv run`; `ruff check src/` — only the pre-existing `orchestrator.py:314` F821 (`search_docs` undefined in HEAD, unrelated); `ruff check` clean on all 12 touched test files; `ast.parse` green on the 3 files with string-target edits. Frozen tests NOT run per testing freeze.
- **Known**: pre-existing `tools/code_search/orchestrator.py:314` calls `search_docs` (defined in `docs.py`) without importing it — committed in HEAD, F821, unrelated to this move. No re-export shims created (clean cutover); stale path references remain only in dated audit docs (`docs/analytics_audit_2026-08-12.md` — historical snapshot, left as-is).

# 2026-09-07 Embeddings: Arctic 384-d cutover — Qdrant Space rebuilt for dense-384 + BM25-IDF
- **Scope**: User directive: rework every embedding-related function to the 384-dim Arctic model via ml/, rebuild the Qdrant HF Space index for dense-384 + sparse BM25, wipe the attached bucket collections.
- **Shipped**:
  - Qdrant Space (chmielvu-web-index.hf.space, storage bucket chmielvu/web-search-index): deleted `web_results` (1024d, 1,351 points) and `web_results_786d` (786d, 2 points) via REST; created `web_results_384d` — named dense `dense` (VectorParams 384-d Cosine) + named sparse `sparse` (SparseIndexParams on_disk=false, `modifier=idf` for BM25-style IDF weighting confirmed against qdrant-client 1.18 `models.Modifier.IDF`).
  - `index/web_results_index.py`: `COLLECTION_NAME` → `web_results_384d`; `COLLECTION_VECTORS` dense size=384; `COLLECTION_SPARSE` adds `modifier=models.Modifier.IDF`. `_ensure_collection` recreates on dim mismatch (unchanged logic).
  - `tools/code_search/snapshot.py`: removed the HF InferenceClient embedding path entirely — `_hf_code_embedding`/`_hf_batch_code_embeddings` now call `ml.embed_query`/`ml.embed_texts` (Arctic 384-d); deleted `HF_CODESEARCH_MODEL`/`HF_FALLBACK_MODEL`, the HF_TOKEN gates, and all `huggingface_hub` imports. Semantic fallback is fail-open (None on service error), same as before; no token needed.
  - `analytics/writers/schema.py`: `_EMBEDDING_DIM` 768 → 384 (query_embeddings/candidate_embeddings now FLOAT[384]); rollover renames old tables to `<name>_<dim>d_legacy`; `model_id` default → `snowflake-arctic-embed-s`.
  - `rerank/models.py`: CandidateEmbedding/RerankEmbeddingContext descriptions now 384-dimensional. `inference/registry.py`: `as_embedding` docstring updated (legacy InferenceClient path retired).
  - Consumers already on ml/ from the earlier consolidation: rerank (bi_encoder, conditional_bi, core), search/service, search/providers/qdrant, inference/adapters/embedding, utils/query_understanding kNN.
- **Verification**: full-suite `ruff check src/` + `ruff format --check src/` clean; dead-ref sweep (InferenceClient/feature_extraction/granite/st-codesearch/old collection names) zero in src; live end-to-end — `embed_texts` 3 docs + `embed_query` 1 query all 384-dim, BM25 sparse OK, 3-point upsert to `web_results_384d`, hybrid RRF query returned correct top hit (cos 0.771 to relevant doc), test points then filter-deleted (collection back to 0 points).
- **Known**: HF MCP bucket files (`collections/web_results*`) mirror the old on-disk state — the Space's live storage rewrites on next use; legacy bucket dirs can be deleted manually if desired. Frozen tests referencing HF embeddings (e.g. tests/test_hf_inference_embeddings.py) are untouched per testing freeze. The 3 live web-search-mcp server processes need a restart to load the new index name.

# 2026-08-25 Process logs SQLite handler — TTL, FTS, exception capture fixes
- **Scope**: Fix the issues found in the process-logs exploration (`duckdb_data/logs/process_logs.sqlite`, 9,160 rows): broken TTL, dead FTS index, always-NULL `exception` column; diagnose the Redis worker reconnect-loop storms.
- **Shipped**:
  - `utils/sqlite_log_handler.py`: TTL cleanup now compares `julianday(recorded_at) < julianday('now') - ttl_hours/24` — the old `datetime()` predicate returned NULL for ISO-8601 text and never matched, so the 48h TTL deleted nothing (88% of rows were older). Cleanup also prunes stale external-content FTS shadow rows.
  - FTS: `_ensure_schema()` backfills `process_logs_fts` on startup; `flush()` feeds new rows via `INSERT ... RETURNING rowid` + `INSERT OR IGNORE INTO process_logs_fts SELECT ... WHERE rowid = ?`. External-content FTS5 tables never auto-populate — MATCH returned 0 while LIKE found 2,496 redis hits.
  - New `TracebackPreservingQueueHandler` (used by `install_process_logging`): stdlib `QueueHandler.prepare()` strips `exc_info`/`exc_text` before enqueueing, which is why the `exception` column was always NULL; the subclass formats first and stashes text as `_exc_text` on the prepared record.
  - Applied the FTS backfill to the live DB: MATCH counts now equal LIKE counts (redis 2496/2496, embedding 492/492). A running server picked up the new code (row count grew to 9,161).
- **Verification**: 5/5 tests in `tests/test_sqlite_log_handler.py` (FTS population, startup backfill, TTL purge incl. FTS pruning, exception persistence) + `test_observability_logging.py` + `test_mcp_startup.py` (9 passed). `ruff check` and `ruff format` clean on both changed files.
- **Known**: Redis (docket worker) healthy — `PING → +PONG` on 6379, `vps-tunnels.service` active since 10:40; the reconnect-loop storms in the logs coincide with tunnel-down windows. No code change needed; no server process running now.

# 2026-08-25 web_search output contract — timeout masking root cause + stale-process skew
- **Scope**: Investigate "web_search output contract not working as designed" — results lacking `link`/`snippet`/`domain`/`published_date`, and `suggested_prompts` missing on the wire.
- **Root cause 1 (fixed)**: `tools/catalog.py::_TOOL_TIMEOUTS["web_search"]` was `60.0s` while the pipeline routinely exceeds it (live campaign p50 45.98s, p95 180.2s; embedding retries mid-run). FastMCP raises `McpError: Tool 'web_search' execution timed out after 60.0s`, which `fastmcp.server.server.call_tool` wraps into `ToolError("Error calling tool 'web_search'")` — clients receive the generic masked envelope (`error_type: "unknown"`, no query/results). The OMP client (`~/.omp/agent/mcp.json`) already waits 120s, so the server killed calls before the client gave up. Bumped to `120.0` (matches `fetch`/`code_search` convention). Verified red→green with a raw stdio probe (`mcp.ClientSession` + `stdio_client` on `.venv/Scripts/python.exe -m kindly_web_search_mcp_server.server`): before = `McpError` at 60s; after = full contract (per-result `link`/`snippet`/`domain`/`published_date` where present, `agent_guidance`, `suggested_prompts`).
- **Root cause 2 (user action)**: the 12:23:57 truncated payload (`title`/`providers`/`provider_count` only, `agent_guidance` present, NO `suggested_prompts`) cannot be produced by current code — `serialize_public_web_search_response` always emits the full allowlist (see `tests/test_public_output_serialization.py`) and `_guide_web_search` always returns `next_prompts=["research_methodology"]` which `_append_enrichment` always appends. `tools/search.py`/`tools/_helpers.py` were modified 12:02:23–12:02:35 on 2026-08-25, ~21 min before that run; MCP stdio processes load code at start, so the serving process ran a pre-12:02 revision. Venv is an editable install (`source = { editable = "." }` in `uv.lock`). Fix: restart the OMP session / kill the stale `web-search-mcp` process.
- **Diagnostic recipe**: `suggested_prompts` presence distinguishes success (always appended) from error (`_guide_error` appends) from stale-code (absent). Raw stdio probe bypasses client rendering; GitNexus `detect_changes` showed 599 changed symbols / 77 files / critical risk (uncommitted working tree).
- **Files touched**: `src/kindly_web_search_mcp_server/tools/catalog.py` (timeout 60→120), `CHANGELOG.md` (`[Unreleased]` entry).
...
- **Verification**: `nx.bipartite.birank` signature + official docs (He et al. 2017; Graph + `top_personalization`); `weighted_projected_graph` forbids MultiGraph; `gh search code nx.bipartite.birank` — almost all vendored NetworkX copies; one real call site `Jose-Velasco/multi-model-recommender` `AddBirank` (also documents AA=0 on opposite partitions). Literature: query–click bipartite suggestion, RAG-Fusion/RRF (already in-stack), QCG-RAG arXiv:2509.21237 (query→neighbor queries→docs).
- **Known**: Academic-search providers returned empty for the BiRank paper this turn — cited via NetworkX docs instead. `should_decompose` still unused. NetworkX unpinned (yake transitive) until Phase 0.

# 2026-08-22b P2 graph feedback loop — plan regrounded on live schema
- **Scope**: Turn the P2 proposal into an implementable plan by verifying every premise against the live `duckdb_data/analytics/search_events.duckdb` and today's code, correcting stale wiki assumptions before any build work.
- **Shipped**: `docs/p2-graph-feedback-loop-plan-2026-08-22.md` (phases 0–5 with acceptance criteria) + wiki retro `p2-graph-loop-plan-regrounded-on-live-65-table-schema`. Key corrections: 65 base tables not 22; `result_labels` exists (14-col DDL, 0 rows — needs writers only); `content_fetches` (1,055 rows, `item_duration_ms` dwell proxy) provides implicit clicks (~25 exact-match links today; canonical join improves); no entity tables anywhere; signal mix is judge-first (1,595 parseable result_quality verdicts vs ~25 clicks). Decisions: bipartite query↔doc graph over `result_catalog` (entities deferred); labels via judge adapter + fetch projector with backfill; ranking consumes a precomputed `graph_features` dict blended in `search/ranking.py::rank_and_finalize` pre-rerank behind `GRAPH_FEEDBACK_ENABLED` (default off) with shadow eval through the empty `ab_*` tables.
- **Verification**: row counts/schema probed live via duckdb read-only sessions; injection point read at `search/ranking.py`; `analytics/feedback.py` confirmed to be drift/SLO tooling (name collision) but its `compute_ndcg_at_10` is reusable; `docket.Cron`/`Perpetual` semantics verified in installed package — they need a running Worker, so nightly-Docket was rejected in favor of lazy refresh (<1 s rebuild at 664-run scale) plus standalone CLI entrypoint.
- **Known**: tree_of_thoughts tool blocked twice this session (thinking-effort 'medium' unsupported by openrouter/stealth/ox-alpha) — branch analysis done inline; client-side `pytz` missing breaks TIMESTAMPTZ reads and `test_convert_pdf_to_markdown` (Phase 0 fix); SCHEMAS.md badly stale (documents 22 of 65 tables); `analytics_sync_state` empty but `motherduck_sync.py` scaffolds a future VPS-hosted rebuild path.
# 2026-08-22 Durable SEP-1686 background tasks — Redis-backed Docket enabled
- **Scope**: Activate the planned Redis Docket backend (P1) so `task=TaskConfig(mode="optional")` tools (`web_search`, `code_search`, `generate_sitemap`, `deep_research`) gain durable, restartable background execution; eliminate the stdio-hang failure class when the backend is unreachable.
- **Shipped**:
  - `.env`: enabled `FASTMCP_DOCKET_URL=redis://127.0.0.1:6379/0`, `FASTMCP_DOCKET_NAME=web-search-mcp`, `FASTMCP_DOCKET_CONCURRENCY=2` (VPS shared Redis 7 via SSH tunnel, per `VPS/Contabo-Zawady` manifest).
  - `src/kindly_web_search_mcp_server/server.py:22-61`: added `_docket_backend_reachable()` (0.75s TCP probe, redis/rediss schemes) and `_resolve_docket_backend()`, executed after `.env` load and before any fastmcp import; unreachable backend downgrades `FASTMCP_DOCKET_URL` to `memory://` with a stderr warning instead of stalling the stdio handshake (root cause of the 2026-08-21 hang incident).
  - WSL `~/.config/systemd/user/vps-tunnels.service`: added `-L 6379:127.0.0.1:6379` (Redis was the only manifest service missing from the forward list); daemon-reload + restart, service active.
- **Verification**: raw RESP `PING` → `+PONG` through tunnel; deterministic 3-branch unit check of the guard (open port keeps redis URL, closed port downgrades, non-redis passes); `tests/test_mcp_startup.py` passes both with backend down (fallback path) and up (durable path); ruff clean on `server.py`.
- **Known**: Full suite `uv run pytest -q` → 14 failed / 1305 passed / 1 skipped / 1 xfailed (~9-11 min; JUnit report at `tmp/full_suite_2026-08-22.xml`). None caused by this change: isolated A/B rerun of all 14 identifiers with the server guard stashed AND docket env commented vs restored gives an identical **4 failed / 12 passed** on both sides (failures: two `tests/cli/test_runtime.py` shutdown-list drifts missing `'telemetry'`, `test_recommendation.py` decomposition/routing heuristics, plus SerpAPI-yahoo/intent-goggles/router-chain/CLI-worker cases that only fail under full-suite ordering pollution). The remaining 10 are full-suite pollution effects, consistent with the 2026-07-22 long-tail notes; tree also carries ~33 pre-existing modified files from parallel sessions (embedding contract, SerpAPI yahoo switch) that several failing tests predate. GitNexus index predates the new symbols (impact tool reports target-not-found); blast radius bounded by grep — both symbols are leaf functions referenced only inside `server.py` module init.
